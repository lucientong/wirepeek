...
static DissectResult<IpInfo> ParseIpv4(std::span<const uint8_t> data) {
  ...
  size_t payload_len =
      std::min(static_cast<size_t>(info.total_length) - header_len, data.size() - payload_offset);
...
static DissectResult<IpInfo> ParseIpv6(std::span<const uint8_t> data) {
  ...
  size_t available_payload =
      std::min(static_cast<size_t>(payload_length), data.size() - kIpv6HeaderLen);
...
std::string FormatIp(const IpAddress& addr) {
  ...
  return fmt::format(
      ...
      "{:02x}{:02x}:{:02x}{:02x}:{:02x}{:02x}:{:02x}{:02x}",
      v6[0], v6[1], v6[2], v6[3], v6[4], v6[5], v6[6], v6[7], v6[8], v6[9], v6[10], v6[11], v6[12],
      v6[13], v6[14], v6[15]);
}
