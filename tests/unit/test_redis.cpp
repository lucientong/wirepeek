...
TEST(RedisParserTest, ParsesIncrementalRequestResponse) {
  ...
  wirepeek::protocol::RedisParser parser(
      [&](const wirepeek::RedisTransaction& transaction) { transactions.push_back(transaction); });
  ...
  parser.Feed(std::span<const uint8_t>(reinterpret_cast<const uint8_t*>(request.data()), 7),
              wirepeek::StreamDirection::kClientToServer, start);
  parser.Feed(std::span<const uint8_t>(reinterpret_cast<const uint8_t*>(request.data() + 7),
                                       request.size() - 7),
              wirepeek::StreamDirection::kClientToServer, start);
  parser.Feed(
      std::span<const uint8_t>(reinterpret_cast<const uint8_t*>(response.data()), response.size()),
...
TEST(RedisParserTest, MarksErrorResponse) {
  ...
  parser.Feed(
      std::span<const uint8_t>(reinterpret_cast<const uint8_t*>(request.data()), request.size()),
