...
- **Protocol Handler tests** (`test_protocol_handler.cpp`): 11 new test cases covering stream lifecycle, protocol detection and routing, unknown protocol fallback, multiple concurrent streams, bidirectional data interleaving.
- **Export writer edge case tests**: 15 new tests covering file creation failure, jumbo payloads, zero-length payloads, special characters, multiple transactions, stdout output, incomplete transactions.
- **TCP reassembler edge case tests**: 7 new tests for sequence number wraparound, memory limits, max concurrent streams, overlapping segments, data after FIN, large out-of-order buffers, FIN from both directions.
- **10 new UiState tests**: Filter, sparkline, stats tracking. Total: 165 test cases.
- **Documentation reorganization**: README refocused as user-facing guide; architecture docs rewritten as developer-facing system design documentation with design decisions, implementation details, and key algorithms (EN + ZH).
...
- nothing, just fix my CI/CD config.
...
[1.0.0]: https://github.com/lucientong/wirepeek/compare/v0.7.0...v1.0.0
